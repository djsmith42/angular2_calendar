...
export function main() {
  ...
  function createMetric(perfLogs) {
    commandLog = [];
    return new Injector([PerflogMetric.BINDINGS, bind(PerflogMetric.SET_TIMEOUT).toValue((fn, millis) => {
    ...
    describe('aggregation', () => {
      ...
      ['script', 'gcTime', 'render'].forEach((metricName) => {
        it(`should support ${metricName} metric`, (done) => {
          aggregate([eventFactory.start(metricName, 0), eventFactory.end(metricName, 5)]).then((data) => {
      ...
      it('should subtract gcTime in script from script time', (done) => {
        aggregate([eventFactory.start('script', 0), eventFactory.start('gc', 1, {'usedHeapSize': 1000}), eventFactory.end('gc', 4, {'usedHeapSize': 0}), eventFactory.end('script', 5)]).then((data) => {
          expect(data['script']).toBe(2);
          done();
        });
      });
      describe('gcTimeInScript / gcAmountInScript', () => {
        it('should detect gc during script execution with begin/end events', (done) => {
          aggregate([eventFactory.start('script', 0), eventFactory.start('gc', 1, {'usedHeapSize': 10000}), eventFactory.end('gc', 4, {'usedHeapSize': 0}), eventFactory.end('script', 5)]).then((data) => {
        ...
        it('should detect gc during script execution with complete events', (done) => {
          aggregate([eventFactory.complete('script', 0, 5), eventFactory.start('gc', 1, {'usedHeapSize': 10000}), eventFactory.end('gc', 4, {'usedHeapSize': 0})]).then((data) => {
            expect(data['gcTimeInScript']).toBe(3);
            expect(data['gcAmountInScript']).toBe(10.0);
            done();
          });
        ...
        it('should ignore gc outside of script execution', (done) => {
          aggregate([eventFactory.start('gc', 1, {'usedHeapSize': 10}), eventFactory.end('gc', 4, {'usedHeapSize': 0}), eventFactory.start('script', 0), eventFactory.end('script', 5)]).then((data) => {
...
//# sourceMappingURL=/Users/patrick/Documents/open source/angular/modules/benchpress/test/metric/perflog_metric_spec.map
