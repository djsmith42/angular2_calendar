...
import {ChangeDetector,
  ...
  PipeRegistry,
  Pipe} from 'angular2/change_detection';
...
export function main() {
  describe('ElementBinderBuilder', () => {
    ...
    function createPipeline({textNodeBindings,
      ...
      return new CompilePipeline([new MockStep((parent, current, control) => {
        ...
        if (isPresent(current.element.getAttribute('viewroot'))) {
          ...
          current.inheritedProtoView = new ProtoView(current.element, new DynamicProtoChangeDetector(normalizeBlank(registry)), new NativeShadowDomStrategy());
    ...
    it('should bind directive properties with pipes', () => {
      ...
      var results = pipeline.process(el('<div viewroot prop-binding directives></div>'));
      var pv = results[0].inheritedProtoView;
      results[0].inheritedElementBinder.nestedProtoView = new ProtoView(el('<div></div>'), new DynamicProtoChangeDetector(registry), new NativeShadowDomStrategy());
      instantiateView(pv);
      evalContext.prop1 = 'a';
      changeDetector.detectChanges();
...
//# sourceMappingURL=/Users/patrick/Documents/open source/angular/modules/angular2/test/core/compiler/pipeline/element_binder_builder_spec.map
